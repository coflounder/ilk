...
Push the directory to a git repository and anyone can adopt it:
...
adopted layer — satisfies the requirement.
...
An unmet requirement is a warning, not a failure. The layer still adopts; checks that
...
Defaults are the point. A layer should adopt cleanly with no prompts and no flags.
...
Adopts the layer into a throwaway repository containing files a human wrote, applies
twice to prove idempotency, drops it, and asserts the repository came back:
...
you/my-pattern 0.1.0
  ...
  ✓ 8 artifact(s)
  ✓ adopt is idempotent
  ✓ drop restores the repository
  · handed over: docs/README.md
```

Layers that fail this cannot be safely tried, and a layer nobody can safely try is one
nobody will adopt. Run it in your own CI.

## Trust

Adopting a layer from a source ilk did not ship requires explicit consent when that
